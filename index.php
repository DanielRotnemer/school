<?php
    ...
    $document = PAGES::NotLoggedInHomePage();
    if (isset($_SESSION['IdNumber'])) { 
        $document = PAGES::LoggedInHomePage();
    }
...
<html>
    <head>
        <title>דף הבית</title>
        <link rel="stylesheet" href="http://<?php echo $GLOBALS['SERVER_ADDRESS']; ?>/index.css?crm=<?php echo time(); ?>"/>
        <script src="https://code.jquery.com/jquery-3.3.1.js"></script>
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
        <script src="http://<?php echo $GLOBALS['SERVER_ADDRESS']; ?>/index.js?crm=<?php echo time(); ?>"></script>
        <?php if (isset($_SESSION['ManagerId'])) {
            echo '<script src="http://'.$GLOBALS['SERVER_ADDRESS'].'/manager.js?crm='.time().'"></script>';
        }  
        ?>      
    </head>
    <body>
