<?php
    ...
    if (!isset($_SESSION['IdNumber']) || !isset($_SESSION['ManagerId'])) 
    {
        header("Location: http://$GLOBALS[SERVER_ADDRESS]/index");
        exit;
    }

    $document = PAGES::CreateCirclePage();

?>

<html>
    <head>
        <title>שיבוץ חוג לשבוע הקרוב</title>
        <link rel="stylesheet" href="http://<?php echo $GLOBALS['SERVER_ADDRESS']; ?>/createCircle.css?crm=<?php echo time(); ?>"/>
        <script src="https://code.jquery.com/jquery-3.3.1.js"></script>
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
        <script src="http://<?php echo $GLOBALS['SERVER_ADDRESS']; ?>/createCircle.js?crm=<?php echo time(); ?>"></script>
